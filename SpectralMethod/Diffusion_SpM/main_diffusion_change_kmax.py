...
def save_data_shifts(N,K,a_bar_min,a_bar_max,a_degger_bar_min,a_degger_bar_max):
	for a_bar in np.arange(a_bar_min,a_bar_max):
		for a_degger_bar in np.arange(a_degger_bar_min,a_degger_bar_max):
			...
			np.save("Data/p_"+str(N)+"_"+str(K)+"_"+"shifts_"+str(a_bar)+"_"+str(a_degger_bar)+".npy",p_mat) 
#save_data_shifts(50,50,6,13,6,13)

#examine the condition of inverted matrices

allcond = [("condition of matrix","N","K","a_bar","a_degger_bar")]
for a_bar in np.arange(6,13):
	for a_degger_bar in np.arange(6,13):
		cond = (max(diffusion_sm_shift_K(N,K,a_bar,a_degger_bar)[1]),N,K,a_bar,a_degger_bar)
		allcond.append(cond)

p_mat = diffusion_sm_shift_K(N,K,a_bar,a_degger_bar)[0]
